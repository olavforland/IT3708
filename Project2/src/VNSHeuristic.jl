...
using ..Objective: time_unfitness_objective, time_fitness_objective
...
function construct_single_route(instance::ProblemInstance, patients::Vector{Patient})
    ...
    solution, obj = local_1_shift(solution, instance, time_unfitness_objective)
    # Iterate until feasibility or max level is reached
    while obj > 0 && level < 5
        # Perform level random 1-shifts
        new_solution = random_1_shift(solution, level)
        # Perform local search
        new_solution, new_obj = local_1_shift(new_solution, instance, time_unfitness_objective)
...
function improve_single_route(route::Vector{Patient}, instance::ProblemInstance)
    ...
    max_iter = 10
    iter = 0
    route = variable_neighborhood_decent(route, instance, time_fitness_objective)
    while (level <= 5) && (iter < max_iter)

        new_route = random_1_shift(route, level)    
        new_route = variable_neighborhood_decent(new_route, instance, time_fitness_objective)

        if time_fitness_objective(new_route, instance) < time_fitness_objective(route, instance)
            level = 1
            route = new_route
        else
            level += 1
